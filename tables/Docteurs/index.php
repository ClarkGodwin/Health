  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="staticBackdropLabel"> Ajout d'un docteur</h1>
      ...
      <form class="modal-body" action="ajout.php" method="POST">
       ...
       <div class="mb-3 row">
        ...
        <div class="col-sm-10">
         <input type="text" required class="form-control" name="specialite">
        </div>
       </div>
       <div class="mb-3 row">
...
<table class="table table-striped">
  ...
  <tbody>
    <?php $i = 0 ;
 while($docteur = $docteurs->fetch_assoc()) :
    $i++ ?>
    <tr>
    <th scope="row"><?= $i ?></th>
    <td><?= $docteur["nom"] ?></td>
...
<div class="modal fade" id="details<?=$i?>" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
	<h1 class="modal-title fs-5" id="staticBackdropLabel">Détails de <?= $docteur["nom"]?></h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
       ...
       <div>
        <p>Spécialité : <?=$docteur["specialite"]?> </p>
       </div>
       <div>
	<p>Centre : <?php foreach($centres as $centre) :
	if($centre['id_centre'] == $docteur['id_centre']){
        echo $centre['nom'];
       } endforeach;?> </p>
       </div>
       <div>
       ...
       <div>
        <p>Telephone : <?=$docteur["telephone"]?> </p>
       </div>
       <div>
...
<div class="modal fade" id="modifier<?=$i?>" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
	<h1 class="modal-title fs-5" id="staticBackdropLabel">Modification de <?= $docteur["nom"]?></h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form class="modal-body" action="modification.php" method="POST">
       <div class="mb-3 row">
        <label for="nom" class="col-sm-2 col-form-label">Nom</label>
        <div class="col-sm-10">
	<input type="text" required class="form-control" name="nom" value="<?=$docteur["nom"]?>">
        </div>
       </div>
       <div class="mb-3 row">
	<label for="prenom" class="col-sm-2 col-form-label">Prénom</label>
        <div class="col-sm-10">
	<input type="text" required class="form-control" name="prenom" value="<?=$docteur["prenom"]?>">
        </div>
       </div>
       <div class="mb-3 row">
        <label for="centre" class="col-sm-2 col-form-label">Centre</label>
	...
	<label for="specialite" class="col-sm-2 col-form-label">Spécialité</label>
        <div class="col-sm-10">
         <input type="text" required class="form-control" name="specialite" value="<?=$docteur["specialite"]?>">
        </div>
       </div>
       <div class="mb-3 row">
        <label for="nom" class="col-sm-2 col-form-label">Adresse</label>
        <div class="col-sm-10">
         <input type="text" required class="form-control" name="adresse" value="<?=$docteur["adresse"]?>">
        </div>
       </div>
       <div class="mb-3 row">
        <label for="phone" class="col-sm-2 col-form-label">Téléphone</label>
        <div class="col-sm-10">
         <input type="number" required class="form-control" name="phone" value="<?=$docteur["telephone"]?>">
        </div>
       </div>
       <div class="mb-3 row">
        <label for="email" class="col-sm-2 col-form-label">Email</label>
        <div class="col-sm-10">
         <input type="email" required class="form-control" name="email" value="<?=$docteur["email"]?>">
        </div>
       </div>
       <input type="hidden" name="id" value="<?=$docteur['id_docteur']?>" >
	<button type="submit" class="btn btn-info soumettre form-control" >Soumettre</button>
      </form>
    </div>
  </div>
...
<div class="modal fade" id="supprimer<?=$i?>" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
	<h1 class="modal-title fs-5" id="staticBackdropLabel">Suppression de <?= $docteur["nom"]?></h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form class="modal-body" action="suppression.php" method="POST">
       <div class="mb-3 row">
        ...
        <div class="col-sm-10">
	<input type="hidden" required class="form-control" name="id" value="<?=$docteur["id_docteur"]?>">
        </div>
       </div>
